...
//process.env['USERPROFILE'].split(path.sep)[2];
//var fileNameFull = 'c:\\Users\\Home\\Documents\\SO_40672979.qvw';
var fileNameFull, fileName, fileNameBase, fileNameTemp, prjFolder;
...
function runQV(callback) {
    require('child_process').exec("qv.vbs " + fileNameFull + ' ' + fileNameBase, function (err, stdout, stderr) {
        if (err) {
            callback();
        }
        callback();
            ...
            .pipe(prompt.prompt({
                ...
                if (res.qvw != 'x) Cancel') {
                    ...
                    if (fileValidated == false) {
                        ...
                        fileNameTemp = './temp/' + fileNameBase;
...
gulp.task('create:xml', ['test'], function () {
    mkdirp.sync(prjFolder);
    del.sync('./temp/' + fileNameBase + '/' + fileName);
...
gulp.task('clear:tempApp', ['test'], function () {
    del(['.\\temp\\' + fileNameBase], function () {
        msg.Success('Temp folder cleared');
    });
});

gulp.task('clear:tempPrj', ['test'], function () {
    del([prjFolder], function () {
        msg.Success('Temp folder cleared');
    });
});

gulp.task('deploy:prod', ['test'], function () {
    deploy('prod');
});

gulp.task('deploy:dev', ['test'], function () {
    deploy('dev');
});

gulp.task('deploy:test', ['test'], function () {
    deploy('test');
});
